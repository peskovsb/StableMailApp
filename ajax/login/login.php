...
if(!USER_LOGGED){
	if(($_POST['user_login'] AND $_POST['user_login']!='') || ($_COOKIE['savelogin'] AND $_COOKIE['savepass'])){
			$_POST['user_login']?$username = $_POST['user_login']:$username = $_COOKIE['savelogin'];
			$_POST['user_password']?$password = $_POST['user_password']:$password = $_COOKIE['savepass'];
			...
			$arrFirstStageValid = $tb->fetch(PDO::FETCH_ASSOC);
			//print_r($arrFirstStageValid);
			...
			if($_POST['user_login'] AND $_POST['user_password']){
				$tb->execute(array(':user_login'=>$username, ':user_password'=>md5(md5(stripslashes($password)).$arrFirstStageValid['salt'])));
			}else{
				$tb->execute(array(':user_login'=>$username, ':user_password'=>$password));
			}
			$arrSecondStageValid = $tb->fetch(PDO::FETCH_ASSOC);
		//echo '<pre>'; print_r($arrSecondStageValid); echo '</pre>';
			if($arrSecondStageValid) {
				if ($arrSecondStageValid['active'] != '0') {
					$_SESSION['user_id'] = $arrSecondStageValid['user_id'];
					$_SESSION['user_login'] = $arrSecondStageValid['user_login'];
					$_SESSION['user_level'] = $arrSecondStageValid['user_level'];
					$_SESSION['active'] = $arrSecondStageValid['active'];
					$_SESSION['user_datereg'] = $arrSecondStageValid['user_datereg'];
					unset($_SESSION['wrongpass']);
					if($_POST['save_the_pass']){
						setcookie("savelogin", $username, time()+(365*24*60*60));
						setcookie("savepass", md5(md5(stripslashes($password)).$arrFirstStageValid['salt']), time()+(365*24*60*60));
					}
				...
				$_SESSION['wrongpass'] = 'Неправильный логин или пароль';
				logout();
			}
	}
}
